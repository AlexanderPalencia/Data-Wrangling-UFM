...
1. Para las siguientes sentencias de `base R`, liste su contraparte de `dplyr`:
    * `str()` -->  Se pueden usar la funcion summarise_all de dplyr dentro de esta se le pude pasar la funcion
    * `df[,c("a","b")]` --> estamos seleccionando las columnas por lo que podemos utilizar la funcion Select de dplyr select(df)
    * `names(df)[4] <- "new_name"` donde la posición 4 corresponde a la variable `old_name` --> Podemos utilizar la funcion rename para renombrar la tabla
